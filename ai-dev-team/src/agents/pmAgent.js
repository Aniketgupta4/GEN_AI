...
// pm agent node function 
// this function is called by langgraph as a node
...
export async function pmAgentNode(state) {
  ...
  // in langgraph -> return new state -> internally handle destructuring of prev state -> next state pe prev state bhejna hai -> bs yaha show ni karray hai destructuring or yaha pe ya phir reference store kara shakte hai prev state ka 
  return {
    pmStatus: "spec_ready",
