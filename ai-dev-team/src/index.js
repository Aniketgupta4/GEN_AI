...
async function main() {
  ...
  // 5. Run the graph
  const threadId = `project-${Date.now()}`; // threadid [unique for all users] --> info kahi store hori hogi need aa id ji jab hm resume karray hai toh next time wahi se start ho
  const config = {
    configurable: {
      thread_id: threadId,
    },
