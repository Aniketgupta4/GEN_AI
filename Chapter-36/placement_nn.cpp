...
//     Hidden Layer  :  16 neurons[how to know kitni hidden layers hongi and kitne neurons honge -> expirement based hai ye toh ]  + ReLU activation[to add non-linearity we use activation function -> but why relu -> Fast, avoids vanishing gradients, introduces non-linearity, and is the default activation for hidden layers]
//                                -> relu gives 10 o/p -> [105,17,-19,......] --> softmax [converts into 0-1] --> [0.1,0.2,....] sum is 1 --> so we check prediction is correct or not ----> if incorrect o/p then cal error backpropagate update w b and all
//     Output Layer  :  10 neurons  + Softmax activation  (10 companies)
//     Loss          :  Cross-Entropy
//     Optimizer     :  Mini-batch Gradient Descent
//
// ----> **** why gpu's[for dedicated tasks but parallely] require ----> kyuki ****parallel pe neurons tasks perform kare samjhe ----> isi liye gpu's require hote hai for these parallel tasks
// ----> **** sune hoge kahi ye model 1-billion parameter pe train hua hai ----> it means ----> w + b ----> jitne bhi hai wahi ka sum hai ye 1 billion
// ----> **** at each epoch data suffle karao then bhejo to next epoch thik
//
...
//
//
// --> **** aysa bhi chiz ban gya hai --> if use online training model then correct model ko bekar kar dega --> corrupt kar dega 
// --> **** Online training ka downside:
//          Wrong or malicious user data can corrupt the model,
//          causing its accuracy to degrade over time.
// =============================================================================
...
// =============================================================================
struct Dataset {
    vector<vector<double>> X;   // features/input  [N x 5]
    vector<int>            y;   // labels/output    [N]   (0..9 = company id)
};
...
// =============================================================================================
//  FEATURE NORMALIZATION (Z-score / Standardization) -> use zscore -2 se 2 ke bich pe aye sab 
//  -----------------------------------------------
...
// =============================================================================
//  TRAIN/TEST SPLIT ****(shuffled data always give for train test)
// =============================================================================
pair<Dataset, Dataset> trainTestSplit(const Dataset& data, double trainFrac) {
    int N = data.X.size();
    vector<int> idx(N);
    iota(idx.begin(), idx.end(), 0);           // [0,1,2,3,4,..........,1999]
    shuffle(idx.begin(), idx.end(), rng);      // [222,554,.................]

    int trainN = (int)(N * trainFrac);
    Dataset train, test;
    for (int i = 0; i < N; i++) {
        if (i < trainN) {       // **** ek sath 2000 data ko suffle is liye ni kare bohot time lagta isi liye --> 1600 and 400 train test pe alag alag suffle kare thik thoda sa less time lagega thik
...
int main(int argc, char** argv) {
    ...
    // Split
    pair<Dataset, Dataset> split = trainTestSplit(data, TRAIN_SPLIT);
